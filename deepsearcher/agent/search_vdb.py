...
#Add some restrictions to queries to avoid massive flooding of contexts.
MAX_DOC_PER_COLL = 10
...
async def search_chunks_from_vectordb(query: str, sub_queries: List[str]):
    ...
    for collection, col_query in collection_2_query.items():
        ...
        for retrieved_result in retrieved_results:
            ...
            if "<think>" and "</think>" in chat_response.content:
                end_of_think = chat_response.content.find("</think>") + len("</think>")
                response_removed_think = chat_response.content[end_of_think+1:]
            else:
                response_removed_think = chat_response.content
            response_removed_think.strip("\n")
            if "YES" in response_removed_think:
                all_retrieved_results.append(retrieved_result)
                accepted_chunk_num += 1
                references.append(retrieved_result.reference)
                log.color_print(
                                        f"<search> {accepted_chunk_num}/{MAX_DOC_PER_COLL} have been selected in current collection.\n"
                            )
                if accepted_chunk_num == MAX_DOC_PER_COLL:
                    log.color_print(f"Maximum doc per collection reached, moving to next collection.")
                    break
